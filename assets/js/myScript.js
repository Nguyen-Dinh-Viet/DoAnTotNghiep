...
$(document).ready(function () {
  ...
  $("span.change_ledger").click(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#change_cat_id option[value='" + data["info"]["cat_id"] + "']").attr(
          "selected",
          "selected"
        );
        ...
        // };
        $(
          "#change_ship_id option[value='" + data["info"]["ship_id"] + "']"
  ...
  $("span.change_shareholder").click(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $(
          "#change_shareholder_id option[value='" + data["idHolder"] + "']"
        ).attr("selected", "selected");
      },
      error: function (
        xhr,
  ...
  $("span.change_transport").click(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#change_ship_id option[value='" + data["idShip"] + "']").attr(
          "selected",
          "selected"
        );
        $("#change_currency option[value='" + data["currency_id"] + "']").attr(
          "selected",
          "selected"
        );
      },
  ...
  $("span.watch_detail").click(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        var stt = 1;
        var title =
          "Thông tin chi tiết chi cho cổ đông: " +
          data[0]["user_lname"] +
          " " +
          data[0]["user_mname"] +
          " " +
        ...
        data.forEach((item) => {
          var temp =
            ...
            stt +
            "</td><td>" +
            formatDateInJS(item["date"]) +
            "</td><td>" +
            item["cate_name"] +
            "</td><td>" +
            formatMoneyInJS(item["money"]) +
            "</td><td>" +
            item["currency_name"] +
            "</td><td>" +
            item["account_name"] +
  ...
  $("span.watch_detail_revenue").click(function () {
    ...
    var data = { id: id, curr: curr, from: from, to: to };
    console.log(data);
    // alert(from);
    $.ajax({
      ...
      success: function (data) {
        ...
        var sum =
          "<tr  style='color: #000;font-size:16px;font-weight:600;'><td colspan='3'>Tổng tiền</td><td>" +
          formatMoneyInJS(sum_collect.toString()) +
          "</td><td>" +
          formatMoneyInJS(sum_spend.toString()) +
          "</td><td></td><td></td></tr>";
        info = info + sum;
        // $('#myTable').append('<tr><td>my data</td><td>more data</td></tr>');
        // $("#modal-watch-detail > tbody:first").append(info);
  ...
  $("span.watch_detail_oil").click(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        var sum =
          "<tr style='color: #000;font-size:18px;font-weight:600;'><td colspan='2'>Tổng tiền</td><td>" +
          formatMoneyInJS(sum_money) +
          "</td><td></td><td></td></tr>";
        info = info + sum;
  ...
  // sự kiện Lưu xe vào bãi
  $("#CarCheckIn #btn_addCarCheckIn").click(function () {
    if ($("input#RFIDcode").val() == 0) {
      $("p#RFIDcode").text("Vui lòng nhập mã thẻ RFID!");
      return;
    }
    if ($("input#license_plates").val() == 0) {
      $("p#license_plates").text("Vui lòng nhập Biển số xe!");
      return;
    }
    if ($("input#TimeIn").val() == 0) {
      $("p#TimeIn").text("Vui lòng nhập thời gian vào!");
      return;
    }
    ...
    // alert(data["TimeIn"]);
    // alert(data["Image_Face_Base64"]);
    $.ajax({
      url: "?mod=homes&action=addCarCheckIn", //Trang xử lý, mặc định trang hiện tại
      method: "POST", //post hoặc get, mặc định là get
      data: data, // dữ liệu truyền lên server
      dataType: "json", // html ,text,script hoặc json
