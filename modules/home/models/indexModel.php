...
function get_all_data_carIn_count_on_days_in_week()
{
   $query = "SELECT  t.DATE, IFNULL(t.XeVao, 0) AS XeVao, week.d 
   FROM (
         SELECT DATE(TimeIn) DATE,
            COUNT(ID) XeVao,
            DATE_FORMAT(TimeIn,'%a') DAYNAME
         FROM accesscontrol
         WHERE DATE(TimeIn) BETWEEN DATE(NOW()) - INTERVAL 1 WEEK AND DATE(NOW())
         GROUP BY DATE(TimeIn)) AS t
   RIGHT OUTER JOIN (
       SELECT 'Sun' AS d
...
function get_all_data_carOut_count_on_days_in_week()
{
   $query = "SELECT  t.DATE, IFNULL(t.XeRa, 0) AS XeRa, week.d 
   FROM (
         SELECT DATE(TimeOut) DATE,
            COUNT(ID) XeRa,
            DATE_FORMAT(TimeOut,'%a') DAYNAME
         FROM accesscontrol
         WHERE DATE(TimeOut) BETWEEN DATE(NOW()) - INTERVAL 1 WEEK AND DATE(NOW())
         GROUP BY DATE(TimeOut)) AS t
   RIGHT OUTER JOIN (
       SELECT 'Sun' AS d
       UNION SELECT 'Mon' AS d
       UNION SELECT 'Tue' AS d
